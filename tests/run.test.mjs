...
test("off-grid AUTO: every field the renderer reads exists and is sane", async () => {
  ...
  assert.equal(p.contract, 12, "payload carries current contract version");
  assert.ok(
  ...
  assert.ok(
    Array.isArray(p.history.pvDaily) && p.history.pvDaily.length === 365,
    "sun strip: one daily-solar entry per day",
  );
  assert.ok(
    Math.max(...p.history.pvDaily) > 3,
    "sun strip peak day is sane (>3 kWh/kW)",
  );
  for (const a of p.auto) {
    ...
    assert.ok(
      a.socNameplatePct && a.socNameplatePct.min.length === 365,
      `${a.chemistry} chart bands cover 365 days`,
    );
    // break-even contract: a number or explicit null — NEVER undefined
    assert.ok(
      typeof a.trueBreakEvenYear === "number" || a.trueBreakEvenYear === null,
...
test("off-grid AUTO honors the independence submenu (tier100 → zero unmet)", async () => {
  const p99 = await runSizing(
    { ...MSG, chemistry: "auto", mode: "offgrid", autoTier: "tier99" },
    { fetchWeather: fakeWeather },
  );
  const p100 = await runSizing(
    { ...MSG, chemistry: "auto", mode: "offgrid", autoTier: "tier100" },
    { fetchWeather: fakeWeather },
  );
  assert.ok(
    p100.auto.every((a) => a.unmetHoursPerYear === 0),
  ...
  for (const chem of ["naion", "lfp", "agm"]) {
    ...
    assert.ok(
      a100.pvKw >= a99.pvKw && a100.battKwh >= a99.battKwh,
      `${chem}: 100% hardware ≥ 99%`,
    );
  }
  assert.ok(
    !p99.autoNote.includes("100% independence"),
    "basis note tracks selection",
  );
...
test("autoNoteFor names exactly the chemistries that solved (count-aware copy)", () => {
  ...
  assert.equal(
    autoNoteFor([{ chemLabel: "Sodium-Ion" }], basis),
    `Sodium-Ion sized for ${basis}`,
  );
  assert.ok(
    !autoNoteFor([{ chemLabel: "Sodium-Ion" }], basis).includes("three"),
  );
...
test("auto payload autoNote is count-aware when all three chemistries solve", async () => {
  ...
  assert.ok(
    !p.autoNote.includes(" of the three"),
    "no stale 'of the three' phrasing",
  );
});
...
test("off-grid SPECIFIC: tier cards + percent history chart bands", async () => {
  ...
  assert.ok(
    p.history.tiers.length >= 2,
    "chart bands traced per solvable tier",
  );
  for (const t of p.tiers.filter((x) => x.solvable)) {
    ...
    assert.ok(
      Number.isFinite(t.paybackYearsLo) && t.paybackYearsLo > 0,
      "payback positive",
    );
  }
...
test("grid-tie SPECIFIC: target fields incl. export economics + chart", async () => {
  const p = await runSizing(
    { ...MSG, chemistry: "naion", mode: "gridtie", exportRate: 0.1 },
    { fetchWeather: fakeWeather },
  );
  ...
  for (const t of p.targets.filter((x) => x.solvable)) {
    ...
    assert.ok(
      t.cutPct >= t.minFraction * 100 - 1,
      `cut honored (${t.cutPct}% vs ${(t.minFraction * 100).toFixed(0)}%)`,
    );
    if (t.exportValueAnnualUsd > 0) sawExportValue = true;
  }
  assert.ok(
    sawExportValue,
    "feed-in credit produces export value on clipped surplus",
  );
});
...
test("grid-tie AUTO honors the bill-cut submenu (cut60 → ~60%)", async () => {
  ...
  for (const a of p.auto) {
    assert.ok(
      a.cutPct >= 59 && a.cutPct <= 63,
      `${a.chemistry} cut ${a.cutPct}% should sit at ~60`,
    );
    assert.ok(
      a.trueBreakEvenYear === null || a.trueBreakEvenYear > 0,
      "break-even sane",
    );
  }
});
...
test("off-grid AUTO carries a full 3×3 options matrix with sane cells", async () => {
  ...
  for (const row of p.matrix.rows) {
    for (const col of p.matrix.cols) {
      ...
      assert.equal(
        cell.unmetHoursPerYear <=
          (col.id === "tier100" ? 0 : col.id === "tier99" ? 87.6 : 438) + 0.1,
        true,
        `${row.id}@${col.id} honors tier budget (${cell.unmetHoursPerYear} h/yr)`,
      );
  ...
  if (t95.solvable && t100.solvable) {
    assert.ok(
      t100.lifetimeCostMid >= t95.lifetimeCostMid,
      "100% tier costs at least the 95% tier",
    );
  }
});
...
test("grid-tie AUTO carries a full 3×3 matrix honoring each cut target", async () => {
  ...
  assert.ok(
    ...
    "matrix includes the clickable 'your target' column",
  );
  ...
  for (const row of p.matrix.rows) {
    for (const col of p.matrix.cols) {
      ...
      assert.ok(cell, `cell ${row.id}:${col.id} exists`);
      if (!cell.solvable) continue;
      const need = col.custom
        ? Math.round(p.customCut.fraction * 100) - 2
        : minBy[col.id];
      assert.ok(
        cell.cutPct >= need,
        `${row.id}@${col.id} cut ${cell.cutPct}% meets ${col.id}`,
      );
    }
  }
});
  ...
  for (const mode of ["offgrid", "gridtie"]) {
    ...
    assert.ok(
      typeof p.bestReason === "string" &&
        p.bestReason.includes(expected.chemLabel),
    ...
    assert.ok(
      Number.isFinite(p.focus.battNameplateKwh) || p.focus.battKwh === 0,
    );
  }
});

test("single-chemistry runs expose focus (rep tier/target) but no best/matrix", async () => {
  const og = await runSizing(
    { ...MSG, chemistry: "lfp", mode: "offgrid" },
    { fetchWeather: fakeWeather },
  );
  ...
  const gt = await runSizing(
    { ...MSG, chemistry: "naion", mode: "gridtie" },
    { fetchWeather: fakeWeather },
  );
...
test("GATE: a bigger bill cut can never show SMALLER 20-year savings (residual-bill sanity)", async () => {
  ...
  const saved20 = (cum) => {
    assert.ok(cum && cum.grid.length === 20, "series present");
    return cum.grid[19] - cum.solar[19];
  };
  ...
  assert.ok(
    s25 < s80 && s80 < s99,
    `savings must rise with cut (25%: ${s25}, 80%: ${s80}, 99%: ${s99})`,
  );
  // The grid line is the full bill, once: 20 × annual spend (within rounding).
  const p = await runSizing(
    { ...MSG, chemistry: "lfp", mode: "gridtie", customCut: 0.8 },
    { fetchWeather: fakeWeather },
  );
  const g19 = p.customTarget.cumCostSeries.grid[19];
  assert.ok(
    Math.abs(g19 - Math.round(p.annualGridSpendUsd * 20)) <= 1,
    `grid line = 20×annual spend (${g19} vs ${Math.round(p.annualGridSpendUsd * 20)})`,
  );
  // And the wedge equals the honest truth: years × displaced bill − true system cost (> 0 at 99% on this sunny fixture).
  assert.ok(
    s99 > 0,
    "99% cut genuinely saves money at a sunny site with a tariff",
  );
});
...
test("off-grid AUTO entries carry the cumulative series the savings panel needs", async () => {
  ...
  for (const a of p.auto) {
    ...
    const rawCross =
      a.cumCostSeries.grid.findIndex((g, i) => g >= a.cumCostSeries.solar[i]) +
      1;
    ...
    assert.equal(
      crossYr,
      a.trueBreakEvenYear,
      `${a.chemistry}: chart crossing == true break-even row`,
    );
  }
});
...
test("incremental cut patch = full engine, minus the parts a cut edit cannot touch", async () => {
  ...
  for (const chemId of ["naion", "lfp", "agm"]) {
    ...
    assert.equal(
      a.pvKw,
      b.pvKw,
      `${chemId}: pv identical (${a.pvKw} vs ${b.pvKw})`,
    );
    assert.equal(a.battKwh, b.battKwh, `${chemId}: battery identical`);
    assert.equal(
      a.lifetimeCostMid,
      b.lifetimeCostMid,
      `${chemId}: lifetime cost identical`,
    );
    assert.ok(
      Array.isArray(a.cumCostSeries && a.cumCostSeries.grid),
      `${chemId}: slice cell carries the series`,
    );
  }
  // Adopted-system capture: SOC bands for the exact requested system.
  assert.ok(
    slice.focusSoc &&
      slice.focusSoc.chemistry === "lfp" &&
      slice.focusSoc.socNameplatePct,
    "focusSoc carries capture bands for the adopted system",
  );
  assert.ok(
    slice.focusSoc.socNameplatePct.min.length > 300,
    "bands cover the whole period",
  );
});
...
test("frontier point details are selection-complete for instant adoption", async () => {
  ...
  for (const pt of p.frontier.points) {
    ...
    assert.ok(
      Number.isFinite(d.battNameplateKwh) && Number.isFinite(d.usableDod),
      "hardware fields",
    );
    assert.ok(
      Number.isFinite(d.servedKwhPerYear) && Number.isFinite(d.lifetimeCostMid),
      "outcome fields",
    );
    assert.ok(
      d.cumCostSeries && d.cumCostSeries.grid.length === 20,
      "cumulative series for the savings panel",
    );
    assert.ok(
      typeof d.trueBreakEvenYear === "number" || d.trueBreakEvenYear === null,
      "break-even present",
    );
  }
});
...
test("GATE: every payload series is a proper stack (emerald ⊂ slate ⊂ amber)", async () => {
  ...
  const entries = [
    p.best,
    ...(p.auto || []),
    ...Object.values((p.matrix && p.matrix.cells) || {}),
  ...
  for (const e of entries) {
    ...
    assert.ok(
      ...
      `${e.chemistry}: system + bills = with-solar`,
    );
    assert.ok(
      Math.abs(bd.saved + bd.withSolar - bd.gridTotal) <= 1,
      `${e.chemistry}: saved + with-solar = grid total`,
    );
    if (Number.isFinite(e.lifetimeCostMid)) {
      assert.ok(
        Math.abs(bd.systemTotal - e.lifetimeCostMid) <= 1,
        `${e.chemistry}: emerald endpoint == card's total 20-year cost`,
      );
    }
    const n = e.cumCostSeries.years - 1;
    assert.ok(
      e.cumCostSeries.system[n] <= e.cumCostSeries.solar[n] &&
        e.cumCostSeries.solar[n] <= e.cumCostSeries.grid[n],
