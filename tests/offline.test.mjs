...
import { buildE1kw, expandProfile, flatProfile, simulate } from "../assets/js/sizing/engine.js";
...
test("bundled profiles exist for every region and carry full 12×24 matrices", () => {
  ...
  for (const p of OFFLINE_PROFILES) {
    ...
    for (const row of p.ghi) { assert.equal(row.length, 24); }
    for (const row of p.tAmb) { assert.equal(row.length, 24); }
...
test("synthesizeFromProfile: 8760 hours, midnight GHI ~0, midday peak sane", () => {
  ...
  for (let m = 0; m < 12; m++) {
    assert.equal(hours[m * 24 * 31 + 0].ghi === undefined ? hours[0].ghi : hours[m * 28 + 0].ghi >= 0, true);
...
test("GATE: offline profile produces plausible annual yield through the real engine", () => {
  ...
  assert.ok(annualKwhPerKw > 1400 && annualKwhPerKw < 2000,
  ...
  const r = simulate({ pvKw: 4, battKwhUsable: 7, e1kw: e1, loadWh: load, chemistry: "lfp", tempsC: temps });
...
test("profile year metadata is a complete recent year", () => {
  ...
  assert.ok(PROFILE_YEAR >= thisYear - 2 && PROFILE_YEAR <= thisYear - 1, `PROFILE_YEAR=${PROFILE_YEAR}`);
});
