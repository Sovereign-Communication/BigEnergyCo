...
test("GATE: rescale ×2 of cached payload ≈ fresh engine run at ×2 load", async () => {
  ...
  assert.equal(
    ...
    "levelized cost is a ratio => preserved",
  );
  ...
  const approx = (a, b, tolPct, what) => {
    ...
    assert.ok(
      rel <= tolPct,
      `${what}: rescaled ${a} vs fresh ${b} (${(rel * 100).toFixed(1)}% off, tol ${tolPct * 100}%)`,
    );
  };
...
test("GATE: rescale stays honest at the regime floor (15 ⇄ 30 kWh/day)", async () => {
  const p15 = await runSizing(
    { ...BASE, dailyKwh: 15 },
    { fetchWeather: fakeWeather },
  );
  const p30 = await runSizing(
    { ...BASE, dailyKwh: 30 },
    { fetchWeather: fakeWeather },
  );
...
test("incremental cut returns the moving recommendation (best/bestReason/focus)", async () => {
  ...
  assert.ok(
    patch.best && patch.best.solvable,
    "patch carries the recommended system",
  );
  assert.equal(
    patch.best.chemistry,
    patch.customCut.best.chemistry,
    "recommendation == custom-target winner",
  );
  assert.equal(patch.best.pvKw, patch.customCut.best.pvKw);
  assert.equal(
    patch.best.cutPct,
    patch.customCut.achievedPct,
    "recommendation cut matches the slider",
  );
  assert.ok(
    typeof patch.bestReason === "string" && patch.bestReason.length > 40,
    "plain-language why",
  );
  assert.equal(
    patch.focus.pvKw,
    patch.best.pvKw,
    "focus follows the recommendation",
  );
  // The recommendation now tracks the slider, not the fixed 80% target.
  assert.notEqual(
    patch.best.cutPct,
    p.best.cutPct,
    "60% slider changes the recommendation away from 80%",
  );
});
...
test("every cumulative series carries the system line, ending on lifetimeCostMid", async () => {
  ...
  assert.ok(
    Array.isArray(series.system) && series.system.length === 20,
    "system series present",
  );
  // Out-of-pocket (solar) sits above the system-only line by the residual bills.
  assert.ok(
    series.solar[19] > series.system[19],
    "wedge between the lines is positive",
  );
  assert.ok(
    series.system[0] < series.solar[0],
    "systems starts below the out-of-pocket line",
  );
  // system endpoint == the card's Total 20-year cost (capex + first labor + swaps).
  assert.ok(
    Math.abs(series.system[19] - p.best.lifetimeCostMid) <= 1,
  ...
  assert.equal(
    be,
    p.best.trueBreakEvenYear,
    "chart crossing == card break-even",
  );
});
