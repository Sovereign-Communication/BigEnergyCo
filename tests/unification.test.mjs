...
import { simulateOffset, simulate, sizeForBillCut, PEAK_HOUR_START, PEAK_HOUR_END } from "../assets/js/sizing/engine.js";
import { buildFrontier } from "../assets/js/sizing/frontier.js";
import { fullRange, landedMidBattKwhFor, costRange } from "../assets/js/sizing/pricing.js";
import {
  cumulativeCostSeries, seriesBreakdown, trueBreakEvenYear, lcoeUsdPerKwh,
  batteryReplacements, lifetimeCostUsd,
} from "../assets/js/sizing/money.js";
import { rescalePayload, scaleRecord, sameSiteOptions } from "../assets/js/sizing/rescale.js";
...
test("UNIFY: fullRange mid always inside [lo, hi], every landedF x chemistry", () => {
  for (const f of [1.0, 1.05, 1.1, 1.35, 1.6]) {
    for (const chem of ["lfp", "naion", "agm"]) {
      for (const [pv, b] of [[5, 0], [0, 10], [8, 20], [0.5, 2]]) {
        const r = fullRange(pv, b, chem, f);
        assert.ok(r.objectiveMid >= r.lo && r.objectiveMid <= r.hi,
...
test("UNIFY: AGM/Na-ion replacement rates exceed LFP; runSizing honors them", async () => {
  assert.ok(landedMidBattKwhFor("agm", 1.1) > landedMidBattKwhFor("lfp", 1.1), "AGM usable-kWh costs more than LFP");
  assert.ok(landedMidBattKwhFor("naion", 1.1) > landedMidBattKwhFor("lfp", 1.1), "Na-ion carries its premium");
  const p = await runSizing({ ...BASE, dailyKwh: 12 }, { fetchWeather: fakeWeather });
  ...
  assert.ok(agm.swapsAndLaborUsd >= impliedMin,
    `AGM swaps $${agm.swapsAndLaborUsd} below AGM-rate floor $${Math.round(impliedMin)}`);
  assert.ok(agm.lifetimeCostMid > lfp.lifetimeCostMid, "AGM true cost exceeds LFP");
});
...
test("UNIFY: battery-only imports cover load PLUS charging losses", () => {
  ...
  const r = simulateOffset({ pvKw: 0, battKwhUsable: 10, e1kw, loadWh: load, chemistry: "lfp" });
  const loadTotal = 500 * n;
  assert.ok(r.importedWh >= loadTotal, `imported ${r.importedWh} must cover load ${loadTotal} (losses add)`);
  assert.ok(r.battWhAc > 0, "peak shifting still happens");
  assert.ok(r.peakLoadWh > 0 && r.peakOffsetFraction > 0, "peak metric reported");
});
...
test("UNIFY: solar and system lines both start at capex+labor; residual exact", () => {
  const s = cumulativeCostSeries({
    capexMidUsd: 10000, annualSavingsUsd: 2000, residualAnnualUsd: 500,
    swapsAndLaborTotalUsd: 4840, replacements: 2, batteryLifeYears: 8, firstLaborUsd: 420,
  });
  assert.equal(s.solar[0] - 500, 10000 + 420, "solar starts at capex + first labor");
  assert.equal(s.system[0], 10000 + 420, "system starts at capex + first labor");
  const bd = seriesBreakdown(s);
  assert.equal(bd.residualBills, 20 * 500, "residual slice is exactly the remaining bills");
  assert.equal(bd.systemTotal + bd.residualBills, bd.withSolar, "stack identity holds");
});

// WS1.5b: break-even counts first labor too.
test("UNIFY: trueBreakEvenYear slips when first labor is counted", () => {
  const base = { capexMidUsd: 5000, annualSavingsUsd: 2500, swapsAndLaborTotalUsd: 0, replacements: 0, batteryLifeYears: 99 };
  const noLabor = trueBreakEvenYear(base);
  const withLabor = trueBreakEvenYear({ ...base, firstLaborUsd: 2400 });
  assert.ok(withLabor >= noLabor, `labor cannot hasten payback (${noLabor} vs ${withLabor})`);
...
test("UNIFY: LCOE includes install labor on first bank and swaps", () => {
  const bare = lcoeUsdPerKwh({ capexMidUsd: 10000, battReplaceCostUsd: 2000, replacements: 2, annualServedKwh: 5000 });
  const full = lcoeUsdPerKwh({
    capexMidUsd: 10000, battReplaceCostUsd: 2000, replacements: 2, annualServedKwh: 5000,
    firstLaborUsd: 420, swapsAndLaborTotalUsd: 4840,
  });
...
test("UNIFY: rescale never touches unit rates; scales marker + envelope", async () => {
  const p1 = await runSizing({ ...BASE, dailyKwh: 20 }, { fetchWeather: fakeWeather });
  const r = rescalePayload(p1, 2);
  const rec = { pvKw: 5, battKwh: 10, battPerKwhLo: 45, battPerKwhHi: 70, costLo: 1000 };
  ...
  if (p1.frontier?.marker) {
    assert.equal(r.frontier.marker.capexUsd, Math.round(p1.frontier.marker.capexUsd * 2), "marker dollars scale with curve");
  }
  if (p1.frontier?.reach && typeof p1.frontier.reach.pvMaxKw === "number") {
    assert.equal(r.frontier.reach.pvMaxKw, Math.round(p1.frontier.reach.pvMaxKw * 2 * 100) / 100, "envelope scales");
  }
});

// WS4.5: site gate respects hardware envelope.
test("UNIFY: sameSiteOptions rejects cross-envelope rescales", () => {
  const a = { latitude: 21.31, longitude: -157.86, mode: "gridtie", chemistry: "lfp", tariff: 0.42, exportRate: null, hardwareConfig: "both" };
  assert.ok(sameSiteOptions(a, { ...a }), "identical inputs match");
  assert.ok(!sameSiteOptions(a, { ...a, hardwareConfig: "battery" }), "battery-only envelope differs");
...
test("UNIFY: battery-only search/card/curve agree on the peak metric", async () => {
  const p = await runSizing(
    { ...BASE, dailyKwh: 15, chemistry: "lfp", mode: "gridtie", hardwareConfig: "battery" },
    { fetchWeather: fakeWeather },
  );
  assert.ok(p.frontier && p.frontier.points.length >= 2, "pure-battery sweep draws a curve");
  for (const pt of p.frontier.points) {
    assert.ok(pt.outcomePct >= 0 && pt.outcomePct <= 100, `peak outcome in range (got ${pt.outcomePct})`);
    assert.equal(pt.battKwh > 0 && pt.pvKw === 0, true, "pure-battery sweep holds battery-only points");
  }
  const solved = (p.targets || []).filter((t) => t.solvable);
  assert.ok(solved.length > 0, "peak targets solve");
  for (const t of solved) {
    assert.ok(t.cutPct >= 0 && t.cutPct <= 100, `card cut in range (got ${t.cutPct})`);
  }
});
...
test("UNIFY: battery-only systems price their inverter (never $0 power electronics)", () => {
  ...
  assert.ok(withInv.objectiveMid > noInv.objectiveMid, "peak-based inverter basis exceeds $0 default");
  assert.ok(withInv.lo > noInv.lo && withInv.hi > noInv.hi, "range legs move together");
});
...
test("UNIFY: grid-tie matrix cells accumulate the FULL bill on the grid line", async () => {
  const p = await runSizing({ ...BASE, dailyKwh: 15 }, { fetchWeather: fakeWeather });
  const cell = p.matrix.cells["lfp:cut80"];
  assert.ok(cell && cell.solvable);
  const expectGrid = Math.round(p.annualGridSpendUsd * 20);
  assert.ok(Math.abs(cell.cumCostSeries.grid[19] - expectGrid) <= 20,
...
test("UNIFY: payload carries contract 11 with load + peak fields", async () => {
  const p = await runSizing({ ...BASE, dailyKwh: 10 }, { fetchWeather: fakeWeather });
