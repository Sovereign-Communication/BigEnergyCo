...
test("PV ladder is ascending, unique, and spans the requested range", () => {
  ...
  assert.ok(
    l[l.length - 1] <= 20 + 1e-9 && l[l.length - 1] > 19,
    "tops out at pvMax",
  );
...
test("paretoFront keeps only systems nothing cheaper beats", () => {
  const pts = [
    { capexUsd: 100, outcome: 0.3 },
    { capexUsd: 150, outcome: 0.25 }, // dearer AND worse - dominated
    { capexUsd: 200, outcome: 0.5 },
    { capexUsd: 210, outcome: 0.5 }, // no better for more money - dominated
    { capexUsd: 300, outcome: 0.7 },
  ];
...
test("thinFront drops unreadable near-duplicates but never the endpoints", () => {
  const front = [];
  for (let i = 0; i < 60; i++)
    front.push({ capexUsd: 100 + i * 50, outcome: 0.3 + i * 0.001 });
  ...
  assert.equal(
    thin[thin.length - 1].capexUsd,
    front[front.length - 1].capexUsd,
    "most capable point survives",
  );
});
...
test("thinFront hides systems too weak to be a real option", () => {
  const front = [
    ...
    { capexUsd: 100, outcome: 0.4 },
    { capexUsd: 200, outcome: 0.9 },
  ];
...
test("findKnee picks the elbow of a concave curve, and abstains on short ones", () => {
  ...
  const front = [
    { capexUsd: 0, outcome: 0.2 },
    { capexUsd: 500, outcome: 0.6 },
    { capexUsd: 1000, outcome: 0.8 },
    { capexUsd: 5000, outcome: 0.9 },
    { capexUsd: 20000, outcome: 1.0 },
  ];
  const k = findKnee(front);
  assert.equal(
    front[k].outcome,
    0.8,
    "elbow is where returns stop being proportional",
  );
  assert.equal(findKnee(front.slice(0, 2)), -1, "no knee in a two-point curve");
  assert.ok(
    costPerPoint(front[2], front[4]) > costPerPoint(front[0], front[2]),
...
test("classifyReach names the shape without inventing prose", () => {
  ...
  assert.ok(
    !/[a-z]{4}\s[a-z]{4}/.test(r.id),
    "verdict is an id, not a sentence",
  );

  const capped = [
    { capexUsd: 0, outcome: 0.2 },
    { capexUsd: 1000, outcome: 0.5 },
    { capexUsd: 30000, outcome: 0.82 },
  ];
  const c = classifyReach(capped, findKnee(capped), {
    pvMaxKw: 30,
    battMaxKwh: 250,
    boundLimited: true,
  });
  assert.equal(
    c.id,
    "beyond-sweep",
    "falling short is reported as a limit of the SEARCH",
  );
  assert.ok(
    c.kneePct !== null,
    "knee economics still reported when full coverage is out of range",
  );
  assert.equal(c.pvMaxKw, 30, "the envelope searched travels with the verdict");
  assert.equal(
    c.battMaxKwh,
    250,
    "so the copy can name it instead of implying physics",
  );
});
...
test("a sunny site produces a monotone curve that reaches full coverage", () => {
  ...
  for (let i = 1; i < f.points.length; i++) {
    assert.ok(
      f.points[i].capexUsd > f.points[i - 1].capexUsd,
      "cost rises along the curve",
    );
    assert.ok(
      f.points[i].outcomePct > f.points[i - 1].outcomePct,
      "coverage rises along the curve",
    );
  }
  assert.ok(
    f.reach.ceilingPct >= 99,
    `Honolulu reaches independence (got ${f.reach.ceilingPct}%)`,
  );
  assert.notEqual(f.reach.id, "beyond-sweep");
  assert.equal(
    f.boundLimited,
    false,
    "a curve that reaches 100% inside the lattice is not bound-limited",
  );
...
test("a dark high-latitude site reports the SEARCH limit, never impossibility", () => {
  ...
  assert.equal(
    f.reach.id,
    "beyond-sweep",
    `Oslo reached ${f.reach.ceilingPct}%`,
  );
  assert.ok(f.reach.ceilingPct < 99, "no false promise of full independence");
  assert.equal(
    f.boundLimited,
    true,
    "the best system sat on the edge of the sweep",
  );
  assert.ok(
    f.reach.pvMaxKw > 0 && f.reach.battMaxKwh > 0,
    "the verdict carries the envelope so the copy can name it",
  );
  assert.ok(
    f.reach.tailCostPerPoint > f.reach.headCostPerPoint,
...
test("REGRESSION: a bound-limited ceiling moves when the sweep widens", () => {
  ...
  assert.ok(
    large.reach.ceilingPct > small.reach.ceilingPct + 1,
...
test("a load small enough to be covered by the smallest system says so", () => {
  ...
  assert.ok(
    f.reach.entryPvKw > 0 &&
      f.reach.entryBattKwh > 0 &&
      f.reach.entryCostUsd > 0,
...
test("the same site at a household load still gets a real curve", () => {
  ...
  assert.notEqual(
    f.reach.id,
    "already-covered",
    "10 kWh/day is a real sizing problem",
  );
...
test("isBoundLimited spots a frontier pinned to the edge of its lattice", () => {
  const pvs = [1, 2, 4],
    bats = [1, 5, 20];
  assert.equal(
    isBoundLimited([{ pvKw: 2, battKwh: 5 }], pvs, bats),
    false,
    "interior point",
  );
  assert.equal(
    isBoundLimited([{ pvKw: 4, battKwh: 5 }], pvs, bats),
    true,
    "PV pinned",
  );
  assert.equal(
    isBoundLimited([{ pvKw: 2, battKwh: 20 }], pvs, bats),
    true,
    "battery pinned",
  );
...
test("grid-tie curves start with solar-only - the cheapest useful first step", () => {
  ...
  assert.ok(
    f.points.some((p) => p.battKwh > 0),
    "storage still appears further along",
  );
});
...
test("off-grid sweeps never propose a system with no battery at all", () => {
  ...
  assert.ok(
    all.every((p) => p.battKwh > 0),
    "an off-grid home with no storage is not a system",
  );
});
  ...
  for (const [mode, chemistry] of [
    ...
    for (const pt of f.points) {
      for (const k of [
        ...
        assert.ok(
          Number.isFinite(pt[k]),
          `${mode}/${chemistry}: point.${k} is a number`,
        );
      }
      assert.ok(
        pt.capexLoUsd <= pt.capexUsd && pt.capexUsd <= pt.capexHiUsd,
        "price band brackets the typical cost",
      );
      assert.ok(
        !("result" in pt),
        "raw simulation objects stripped before postMessage",
      );
    }
    assert.ok(
      [
    ...
    assert.equal(
      typeof f.boundLimited,
      "boolean",
      "every payload says whether the sweep ran out of road",
    );
    if (p.focus) {
      assert.ok(
        f.marker,
        `${mode}/${chemistry}: the option being read is marked`,
      );
      assert.ok(
        Number.isFinite(f.marker.capexUsd) &&
...
test("AUTO: an unreachable cut target falls back to the nearest workable cut, never a blank grid", async () => {
  ...
  assert.ok(
    p.auto && p.auto.length > 0,
    "the visitor still gets battery/solar cards to compare",
  );
  assert.equal(
    p.autoFallback,
    true,
    "the fallback is flagged so the UI can say so",
  );
  assert.equal(
    p.effectiveTargetId,
    "cut80",
    "the nearest reachable cut is used",
  );
  assert.ok(
    p.frontier && p.frontier.marker,
    "the recommended option is still marked on the curve",
  );
  for (const a of p.auto)
    assert.ok(
      a.cutPct >= 55 && a.cutPct <= 90,
      `honest cut ${a.cutPct}% (was 0 / -100 before the fix)`,
    );
});
