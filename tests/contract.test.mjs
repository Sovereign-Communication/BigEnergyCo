...
function readUiContract() {
  const src = fs.readFileSync(path.join(__dirname, "..", "assets/js/sizing/ui.js"), "utf8");
...
test("ui.js PAYLOAD_CONTRACT matches run.js PAYLOAD_CONTRACT", () => {
  ...
  assert.equal(ui, PAYLOAD_CONTRACT,
...
test("sizing payload actually carries the contract version", async () => {
  ...
  const { OFFLINE_PROFILES, PROFILE_YEAR } = await import("../assets/js/sizing/profiles.js");
  ...
  const p = await runSizing(
    { latitude: 21.31, longitude: -157.86, dailyKwh: 10, tariff: 0.42, chemistry: "lfp", mode: "offgrid" },
    { fetchWeather: fakeWeather },
  );
  assert.equal(p.contract, PAYLOAD_CONTRACT, "payload carries the current contract version");
});
