...
import { DAYS_PER_MONTH, costRange, fullRange, CURRENCIES } from "../assets/js/sizing/pricing.js";
...
function fakeWeather(lat, lon) {
  const p = OFFLINE_PROFILES.find(pp => pp.name.includes("Honolulu")) || OFFLINE_PROFILES[0];
  return { hours: synthesizeFromProfile(p), meta: { latitude: lat, longitude: lon, startYear: 2023, endYear: 2023, years: 1, source: "fake", timeStandard: "LST" } };
}

test("DAYS_PER_MONTH is 30.44 and bill<->kWh round-trips", () => {
  assert.equal(DAYS_PER_MONTH, 30.44);
  const bill = 150, rate = 0.28;
...
test("FX: display tariff round-trips via USD", async () => {
  ...
  const base = await runSizing({ latitude: 21.31, longitude: -157.86, dailyKwh: 10, chemistry: "lfp", years: 1, tariff: 0.42, mode: "offgrid" }, deps);
  const viaFx = await runSizing({ latitude: 21.31, longitude: -157.86, dailyKwh: 10, chemistry: "lfp", years: 1, tariff: display/fx, mode: "offgrid" }, deps);
...
test("export-rate input is converted like the tariff (display currency in, USD out)", () => {
  const ui = readFileSync(new URL("../assets/js/sizing/ui.js", import.meta.url), "utf8");
  assert.match(ui, /exportRate: \(\(\) => \{[\s\S]*?v \/ fx\.rate[\s\S]*?\}\)\(\)/);
  assert.match(ui, /label\[for="exportRate"\] span/);
});

test("LCOE fields present for all solvable tiers after bugfix", async () => {
  const deps = { fetchWeather: async () => fakeWeather(21.31, -157.86) };
  const p = await runSizing({ latitude: 21.31, longitude: -157.86, dailyKwh: 10, chemistry: "lfp", years: 1, tariff: 0.42, mode: "offgrid" }, deps);
  for (const t of p.tiers) if (t.solvable) assert.ok(Number.isFinite(t.lcoeUsdPerKwh), `tier ${t.id} missing lcoe`);
  const pg = await runSizing({ latitude: 21.31, longitude: -157.86, dailyKwh: 10, chemistry: "lfp", years: 1, tariff: 0.42, mode: "gridtie" }, deps);
  for (const t of pg.targets) if (t.solvable) assert.ok(Number.isFinite(t.lcoeUsdPerKwh), `target ${t.id} missing lcoe`);
  const pa = await runSizing({ latitude: 21.31, longitude: -157.86, dailyKwh: 10, chemistry: "auto", years: 1, tariff: 0.42, mode: "offgrid" }, deps);
  for (const a of pa.auto) if (a.solvable) assert.ok(Number.isFinite(a.lcoeUsdPerKwh), `auto ${a.chemistry} missing lcoe`);
});
