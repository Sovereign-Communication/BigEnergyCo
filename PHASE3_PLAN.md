...
**The problem, stated precisely.** Every view in the tool today answers _one_
...
_how far can I get, and what does each further step cost?_ Nothing on the site
...
site can finally say: _Honolulu reaches full independence for about $3,100. At
Oslo, the largest system this tool sizes still leaves you short — and the last
few percent cost more than twenty times what the first ones did._
...
| 4. Best first dollar                              | not started                                                                       | high, but ranks _actions_; the frontier already exposes the cheapest useful step (solar-only shows up as the first grid-tie point) | next                |
...
- `ui.js`: renders the panel, feeds the verdict to the printable one-pager and
  to the advisor brief (so the model explains _these_ numbers, never its own),
...
- **Ranking actions** (efficiency, load-shifting, generator) — that is phase 4.
  The frontier ranks _systems_; it does not yet know that a new fridge might
  beat both.
...
was the limit _at any price_. It was not. It was the top-right corner of the
...
The bill-cut percentage — on the curve _and_ on the cards — is computed as
...
panel prints: _"Sizing is not your constraint here. The smallest practical
system — 0.4 kW of panels and 1 kWh of battery, about $304 — already covers
this entire load, year-round."_ For a phone-and-lights visitor that is the
