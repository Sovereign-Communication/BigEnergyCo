...
| Area                  | Measurement                                                                                                                                                                                                                                                |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Tests                 | `npm test`: 739 / 739 pass (~148 s) at `b60a651`. The adoption PR adds 12 tests, so the same command reads 751 on its head. `check-chars`, `npm run seo`, prettier and `deploy:check` all pass.                                                            |
...
- Six plain-language tiles in one vocabulary. The first is pre-selected:
  ...
  6. Two toggles inside tiles 1, 2 and 4: **Keep a backup reserve in my battery** (`reservePct`) and **Also keep essentials running in a power cut**, which runs the R-UC-03 outage simulator on the same sized system and adds outage coverage % to that result
...
| V-11 | The numbers behind the Jev pricing. The owner reports ~2 billion tokens consumed for US$0.07. At the canonical US$0.00042 per million tokens that would be ~US$0.00084, so the two figures do not reconcile. The canonical constant stands (D-13); the monthly cost roll-up (R-AI-08) records the provider-billed amount next to the computed one, so a discrepancy becomes visible. P0.3 exits only after one week of live gate traffic has been reconciled against TypeSafe's billed amount (no cap is added, per D-13). | TypeSafe billing                               | P0.3        |
