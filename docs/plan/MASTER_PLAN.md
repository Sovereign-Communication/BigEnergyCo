...
| D-10 | **Legality.** All four paths are always computed. Each is flagged per country as _Allowed_, _Allowed, but loses incentive X_, _Not permitted for grid connection_, or _Unverified: check locally_, with a cited source. A lost incentive is priced into that path.                                                                                                                                                                                                                                                                                                                                                                  |
...
The owner asked for **99/100 on every aspect**. A metric scored out of 100 must reach ≥ 99. Anything else must meet the numeric threshold below. Thresholds are deliberately **stricter than Google's own "good" lines** (for example, Core Web Vitals "good" is LCP ≤ 2.5 s, INP ≤ 200 ms and CLS ≤ 0.1 at p75).
...
| Jev scoped gate (Q-01)                 | P0                        | P0 (on the phase's own facets)                        |
...
| Area                  | Measurement                                                                                                                                                                                                                                                |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Tests                 | `npm test`: 739 / 739 pass (~148 s). `check-chars`, `npm run seo`, prettier and `deploy:check` all pass.                                                                                                                                                   |
...
| Coverage of the world | 220 per-country place files and 66 city pages, but tariffs come from ~40 lat/lon boxes plus 50 US states and DC (`assets/js/sizing/pricing.js:229-704`). 24 currencies. 6 locales, with `ar` the only RTL one.                                             |
...
| F-11 | S2  | Export credit is a per-hour clip-and-credit with no monthly or annual netting ("banking") or true-up (`engine.js:749-765`).                                                                     | R-UC-01              |
...
| ID   | Sev | Finding                                                                                                                                                                                                                                                                    | Closed by            |
| ---- | --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- |
| F-34 | S1  | Advisor replies are not verified. There is no per-reply timestamp. Sizing context is embedded as bracketed text inside the user message (`assets/js/chat.js:230-330`, `worker/index.js:503-694`).                                                                          | R-AI-01..06          |
...
| F-37 | S2  | The Jev gate targets 95, not 99. Its price constant is 42.0, not 0.00042 (D-13). The OpenRouter fallback is live in `worker/index.js:36-37,224-263` and `scripts/validate-jev-complete.mjs:50`. 13 of 16 facets default to `mixed` without a live call. No CI job runs it. | P0.3                 |
...
| F-40 | S2  | About 14 legacy plan and audit documents at the repo root give conflicting guidance to contributors and agents.                                                                                                                                                            | P0.2                 |
...
| F-43 | S3  | Location defaults are overridden only by retyping fields. There is no explicit "confirm what we inferred" step (`ui.js:7150-7171`).                                                                                                                                        | R-FLOW-02            |
...
- Six plain-language tiles in one vocabulary. The first is pre-selected:
  ...
  6. (a toggle inside tiles 1 and 2) **Keep a backup reserve in my battery**
- The internal representation is a single `useCase` enum plus an optional `reservePct`. The legacy `mode` × `hardwareConfig` pair is derived from it inside the engine adapter and never shown to the visitor (closes F-17).
...
- **R-UC-01**: Export compensation models the regime from the registry: net metering with a banking period (monthly or annual true-up), net billing (an export rate below the retail rate), a feed-in tariff, or zero export. Fixtures cover each regime.
...
### 6.4 The four-path economics (R-PATH-01..08)

All amounts are in the visitor's display currency, converted from each registry value's source currency and year (§6.8). The horizon is years 1–20, and the install happens at year 0. Amounts are **nominal by default**; the "today's money" toggle deflates them by the registry inflation rate. `S` is a physical system (PV kWp, usable battery kWh, inverter kW). `E_y` is its year-y energy, with degradation (registry, cited). The physics for a given `S` is **shared by every path**; only the money differs.
...
- **R-CHART-05: mobile.** Legible at 320 px: ≤ 5 series, tick labels ≥ 12 px, and no overlapping direct labels (collision-avoidance tested).
...
6. **R-RES-06: use-case block.** Backup hours, the reserve trade-off, off-grid autonomy and generator use, or portable runtime.
...
- **R-AI-03: verdict handling.** A **pass** shows no badge; "Checked by Jev at {time}" appears in Assumptions & sources. A **flag** triggers a deterministic re-check (a re-run with the alternate weather source where available, and a registry bounds re-validation). If the re-check clears it, the result is revealed with a note. If not, the flag's reason is shown as a notice on the affected numbers. **Unavailable** is a muted note. Jev **never** changes a number.
...
- **R-AI-05: per-reply verification**, in the worker, in this order:
  ...
  4. Return `{reply, model, continuations, respondedAt (ISO-8601 UTC), verification: {available, truthfulness, accuracy, numericCheck: {checked, unmatched}, regenerated, jevModel}}`.
...
- **R-AI-07: one trust vocabulary.** Only two phrases exist: "Checked by Jev" (advisor replies, and inside Assumptions & sources) and "flagged". Words like "verified", "safe", "certified" or "accurate" are never used for AI checks. A lint enforces this in the locale sources.
...
- **R-DATA-02: coverage.** Every one of the 249 ISO 3166-1 codes has currency, tariff, fixed charge, export regime, legality flags for the 4 paths, turnkey benchmark, hardware landed factor, electrician rate, permit fees and lease availability. **"Unknown" is a permitted value.** Unknown triggers the Step-2 "check your bill" prompt, or a wide grade-D band from a published derivation formula. Unknown values are never silently guessed.
...
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------- |
| P0.1 | **Adopt this plan**: plan, lock, amendment log, ledger, `check-plan-pin` in CI, `docs/plan/**` made PR-only in the push guards, PR template, CODEOWNERS for `docs/plan/`, AGENTS.md precedence note, tracking issue.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | The adoption PR merged; `adopted` ledger row                |
...
| P1.5 | Heatmap: self-host Leaflet, guard the global, and show a message on failure.                                                                                                                                                                                                            | F-28                       |
...
| Item | Work                                                                                                                                                                               |
| ---- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P2.1 | Registry schema, `scripts/build-registry.mjs`, `data:check` (schema, grades, licences, `review_by`), and tests. Wired into `npm run seo` and the weekly scheduled job (R-DATA-07). |
| P2.2 | Verify the Appendix C seeds at their primary sources (V-01..V-04, V-09) and populate A/B data for the top markets by solar need and traffic.                                       |
...
| P2.6 | Point the live site's `pricing.js` adapters at the registry. The existing UI now shows registry-backed numbers and sources.                                                        |
...
| P3.2 | Incentives, escalation, O&M and replacements in `money.js` (R-PATH-05, R-PATH-06), plus tests.                                 |
...
| P3.7 | Fixture matrix and goldens (R-ENG-02); determinism tests (R-ENG-04); the PVWatts validator (R-ENG-03, Q-11).                   |
...
| P5.2 | Components (R-DS-02) and `/design/` (R-DS-06), with visual baselines (Q-09 absolute from here).                                                                                                      |
| P5.3 | The shared shell on about, 404, blog, city, heatmap, sources and methodology (R-DS-05). Lighthouse and axe become absolute for these templates.                                                      |
| P5.4 | i18n runtime: per-locale files, `Intl` formatting, the script-direction table, logical CSS and glyph-coverage tests (R-I18N-01..04). The 6 existing locales are migrated with no loss (parity test). |
...
| Item | Work                                                                                                                                                                                                                                                                                                                                                    |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P6.1 | **Slice 1, bill-cut end to end.** Steps 1–5, results R-RES-01..08 for bill-cut, the frontier and timeline (R-CHART-01..05), verify-then-reveal (R-AI-01..03), share v2 (R-FLOW-06), print and CSV. Budgets absolute for `/next/`.                                                                                                                       |
...
| P9.4 | Structured data (R-SEO-02) and E-E-A-T surfaces (R-SEO-05).                                          |
...
**Size estimate** (for sequencing, not a promise): P0 ~6 PRs, P1 ~6, P2 ~8, P3 ~8, P4 ~6, P5 ~8, P6 ~12, P7 ~4, P8 ~5, P9 ~6, P10 ~2; P11 is ongoing.
...
6. Attach the scoped live Jev report (Q-01). Before O-01, the owner runs it locally and attaches it.
...
| V-11 | The numbers behind the Jev pricing. The owner reports ~2 billion tokens consumed for US$0.07. At the canonical US$0.00042 per million tokens that would be ~US$0.00084, so the two figures do not reconcile. The canonical constant stands (D-13); the monthly cost roll-up (R-AI-08) records the provider-billed amount next to the computed one, so a discrepancy becomes visible. | TypeSafe billing                               | P0.3        |
