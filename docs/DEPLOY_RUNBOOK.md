...
| --------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| A run sits `queued` for tens of minutes with no runner ever allocated | 2026-09-19 — ~45 min, unstuck by hand with cancel + rerun                                                                              | `.github/workflows/workflow-watchdog.yml` (hourly) runs `scripts/unstick-queued-runs.mjs`, applying that same remedy automatically. Hourly rather than every 15 minutes on purpose: nothing user-facing waits on a stalled run, and each tick costs a runner slot, so `*/15` would spend 96 runs a day on a rare event. During a total runner outage this job is queued too, so it only ever helps a partial stall — the case actually observed. Every tick FIRST runs `--check-permissions`, a side-effect-free probe (it aims a cancel at a run id that cannot exist, so an authorised token is simply told 404 while an unauthorised one gets 403) — because a scheduled run's token is a different credential from an operator's `gh` login, a passing dry run would prove nothing about the act path. If the token cannot cancel, the job FAILS rather than sitting there looking healthy. The selection guards are all enforced in `scripts/lib/stuck-runs.mjs` and pinned by `tests/ci-resilience.test.mjs`: only `main` runs are candidates (a cancelled PR check run would shadow a passing one), only runs queued past 20 minutes, never a run whose own workflow has one in progress (that is legitimate concurrency queueing, not a stall), at most 3 per pass, and a full stand-down above 8 stuck runs because that is a platform incident where cancelling would amplify the load instead of clearing it. |
