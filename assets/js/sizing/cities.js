...
import { usStateCode, US_STATES } from "./pricing.js?v=20260908a";
  ...
  const promise = (async () => {
    ...
    try {
      const response = await fetchImpl(
        "./assets/js/sizing/city-data/index.json?v=20260908a",
        { cache: "force-cache" },
      );
        ...
        index.map(async (item) => {
          ...
          try {
            const part = await fetchImpl(
              `./assets/js/sizing/city-data/${encodeURIComponent(stem)}.json?v=20260908a`,
              { cache: "force-cache" },
            );
            if (part.ok) {
