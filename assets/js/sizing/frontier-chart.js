...
export function chartBox(hostWidth) {
  ...
  const ratio = w < 520 ? 0.78 : 0.60;
  return {
    W: w,
    H: clamp(Math.round(w * ratio), 250, 430),
    pad: { l: narrow ? 44 : 58, r: narrow ? 12 : 18, t: narrow ? 16 : 22, b: narrow ? 44 : 50 },
    font: narrow ? 11.5 : 13,
...
export function axisTicks(max, count = 5) {
  ...
  let step = 0;
  outer:
  for (let k = exp - 1; k <= exp + 2; k++) {
    for (const m of [1, 2, 2.5, 5]) {
      const cand = m * Math.pow(10, k);
      if (cand > 0 && max / cand <= count + 1e-9) { step = cand; break outer; }
    }
  }
...
export function renderFrontier(host, frontier, opts = {}) {
  ...
  const VB_W = box.W, VB_H = box.H, PAD = box.pad;
  const FS = box.font, FS_TAG = box.fontTag;
  ...
  for (const pct of [0, 25, 50, 75, 100]) {
    const y = Y(pct);
    push(`<line x1="${PAD.l}" y1="${y.toFixed(1)}" x2="${(VB_W - PAD.r).toFixed(1)}" y2="${y.toFixed(1)}" stroke="${C.grid}" stroke-width="1"/>`);
    push(`<text x="${(PAD.l - (box.narrow ? 7 : 10)).toFixed(1)}" y="${(y + 4).toFixed(1)}" text-anchor="end" font-size="${FS}" fill="${C.text}">${pct}%</text>`);
  }
  for (const usd of axisTicks(xMax, box.xTicks)) {
    const x = X(usd);
    push(`<line x1="${x.toFixed(1)}" y1="${PAD.t}" x2="${x.toFixed(1)}" y2="${(VB_H - PAD.b).toFixed(1)}" stroke="${C.grid}" stroke-width="1"/>`);
    const isLast = usd >= xMax - 1e-9;
    push(`<text x="${(isLast ? x + 4 : x).toFixed(1)}" y="${(VB_H - PAD.b + 20).toFixed(1)}" text-anchor="${isLast ? "end" : "middle"}" font-size="${FS}" fill="${C.text}">${esc(money(usd))}</text>`);
  }
  push(`<line x1="${PAD.l}" y1="${PAD.t}" x2="${PAD.l}" y2="${(VB_H - PAD.b).toFixed(1)}" stroke="${C.axis}" stroke-width="1"/>`);
  push(`<line x1="${PAD.l}" y1="${(VB_H - PAD.b).toFixed(1)}" x2="${(VB_W - PAD.r).toFixed(1)}" y2="${(VB_H - PAD.b).toFixed(1)}" stroke="${C.axis}" stroke-width="1"/>`);
  ...
  if (knee) {
    ...
    push(`<rect x="${kx.toFixed(1)}" y="${PAD.t}" width="${(VB_W - PAD.r - kx).toFixed(1)}" height="${plotH.toFixed(1)}" fill="${C.tailWash}"/>`);
  }

  // ── price-uncertainty band (cheap DIY sourcing .. shipped retail) ──────
  const bandFwd = pts.map((p) => `${X(p.capexLoUsd ?? p.capexUsd).toFixed(1)},${Y(p.outcomePct).toFixed(1)}`);
  const bandBack = [...pts].reverse().map((p) => `${X(p.capexHiUsd ?? p.capexUsd).toFixed(1)},${Y(p.outcomePct).toFixed(1)}`);
  push(`<polygon points="${bandFwd.concat(bandBack).join(" ")}" fill="${C.band}"/>`);
  ...
  if (sweepLimited) {
    ...
    push(`<line x1="${PAD.l}" y1="${y.toFixed(1)}" x2="${(VB_W - PAD.r).toFixed(1)}" y2="${y.toFixed(1)}" stroke="${C.ceiling}" stroke-width="1.5" stroke-dasharray="6 4"/>`);
    ...
    push(`<text x="${(VB_W - PAD.r - 6).toFixed(1)}" y="${tagY.toFixed(1)}" text-anchor="end" font-size="${FS}" font-weight="700" fill="${C.ceiling}">${esc(t("frontierCeilingTag", { pct: reach.ceilingPct }))}</text>`);
  }

  // ── knee marker ───────────────────────────────────────────────────────
  if (knee) {
    const kx = X(knee.capexUsd), ky = Y(knee.outcomePct);
    push(`<line x1="${kx.toFixed(1)}" y1="${ky.toFixed(1)}" x2="${kx.toFixed(1)}" y2="${(VB_H - PAD.b).toFixed(1)}" stroke="${C.knee}" stroke-width="1.2" stroke-dasharray="4 4"/>`);
    push(`<circle cx="${kx.toFixed(1)}" cy="${ky.toFixed(1)}" r="6" fill="none" stroke="${C.knee}" stroke-width="2.4"/>`);
    ...
    if (!box.narrow) {
      ...
      const at = placeLabel(kx, label, FS_TAG, { left: PAD.l + 2, right: VB_W - PAD.r - 2 }, 8);
      push(`<text x="${at.x.toFixed(1)}" y="${(VB_H - PAD.b - 9).toFixed(1)}" text-anchor="${at.anchor}" font-size="${FS_TAG}" font-weight="700" fill="${C.knee}">${esc(label)}</text>`);
    }
  }
  ...
  const fitFont = (text, run) =>
    Math.max(9.5, Math.min(FS_TAG, run / Math.max(1, String(text).length * 0.56)));
  const xFs = fitFont(xLabel, plotW);
  const yFs = fitFont(yLabel, plotH);
  push(`<text x="${(PAD.l + plotW / 2).toFixed(1)}" y="${(VB_H - (box.narrow ? 6 : 8)).toFixed(1)}" text-anchor="middle" font-size="${xFs.toFixed(1)}" fill="${C.textBright}">${esc(xLabel)}</text>`);
  push(`<text transform="translate(${box.narrow ? 12 : 15} ${(PAD.t + plotH / 2).toFixed(1)}) rotate(-90)" text-anchor="middle" font-size="${yFs.toFixed(1)}" fill="${C.textBright}">${esc(yLabel)}</text>`);

  const titleId = "frontierSvgTitle";
  const descId = "frontierSvgDesc";
  const desc = t("frontierSvgDesc", {
    n: pts.length,
    lowCost: money(pts[0].capexUsd), lowPct: pts[0].outcomePct,
    highCost: money(pts[pts.length - 1].capexUsd), highPct: pts[pts.length - 1].outcomePct,
  });
  ...
  const legendItems = [
    [swatch(`<span style="position:absolute;top:4px;left:0;width:22px;height:2.5px;background:${C.curve};border-radius:2px;"></span>`), t("frontierLegendCurve")],
  ...
  const legend =
    ...
    legendItems.map(([sw, label]) => `<li style="display:flex;align-items:center;">${sw}<span>${esc(label)}</span></li>`).join("") +
    `</ul>`;
...
export function frontierVerdict(frontier, opts = {}) {
  ...
  const params = {
    ...
    ceilingCost: r.ceilingCostUsd === null || r.ceilingCostUsd === undefined ? "" : money(r.ceilingCostUsd),
    kneePct: r.kneePct,
    kneeCost: r.kneeCostUsd === null || r.kneeCostUsd === undefined ? "" : money(r.kneeCostUsd),
    headCost: r.headCostPerPoint === null || r.headCostPerPoint === undefined ? "" : money(r.headCostPerPoint),
    tailCost: r.tailCostPerPoint === null || r.tailCostPerPoint === undefined ? "" : money(r.tailCostPerPoint),
    ratio: r.tailRatio,
    pvMax: r.pvMaxKw ?? "",
    battMax: r.battMaxKwh ?? "",
    pv: r.entryPvKw ?? "",
    batt: r.entryBattKwh ?? "",
    cost: r.entryCostUsd === null || r.entryCostUsd === undefined ? "" : money(r.entryCostUsd),
  };
  const suffix = gridTie ? "Grid" : "Offgrid";
  if (r.id === "already-covered") return t("frontierVerdictCovered" + suffix, params);
  if (r.id === "beyond-sweep") return t("frontierVerdictBeyondSweep" + suffix, params);
