...
import {
  buildE1kw, flatProfile, expandProfile, simulate, sizeAllTiers,
...
console.log(`Site ${lat},${lon} · load ${dailyKwh} kWh/day · fetching NASA POWER...`);
const t0 = Date.now();
const { hours, meta } = await fetchHourlySeries({ latitude: lat, longitude: lon, years: 5 });
console.log(`Got ${hours.length} hourly records (${meta.startYear}–${meta.endYear}) in ${((Date.now() - t0) / 1000).toFixed(1)}s`);
...
for (const chem of ["lfp", "naion"]) {
  ...
  const results = sizeAllTiers({
    e1kw, loadWh, tempsC, chemistry: chem, years,
    costPerWpv: 0.35, costPerKwhBatt: chem === "lfp" ? 140 : 160,
    battMax: 150,
  });
  for (const { tier, sizing } of results) {
    if (!sizing) { console.log(`  ${tier.label}: NO SOLUTION within search bounds`); continue; }
