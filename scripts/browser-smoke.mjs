...
const gate = (name, ok, extra = "") => {
  console.log(`${ok ? "SMOKE OK    " : "SMOKE FAIL  "}${name}${extra ? " — " + extra : ""}`);
  if (!ok) failures++;
};

const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
const isCsp = (e) => /Content Security Policy|Refused to (connect|load)|violates .* directive/i.test(e);
  ...
  try {
    ...
    ws.onmessage = (ev) => {
      ...
      if (msg.id && pending.has(msg.id)) {
        ...
        errors.push(`page exception: ${msg.params?.exceptionDetails?.text || JSON.stringify(msg.params).slice(0, 300)}`);
    ...
    const navigate = async (url) => {
      const loaded = new Promise((res) => {
        ...
        ws.onmessage = (ev) => {
          prev(ev);
          try { if (JSON.parse(ev.data).method === "Page.loadEventFired") { ws.onmessage = prev; res(); } } catch { /* noop */ }
        };
      });
    ...
    const chooseHonolulu = async () => {
      await evaluate(`(() => { const s = document.getElementById("citySearch"); s.focus(); s.value = "Honolulu"; s.dispatchEvent(new InputEvent("input", { bubbles: true })); return true; })()`);
      const ok = await poll(async () =>
        (await evaluate(`document.querySelectorAll('#citySuggestions [role="option"]').length`)) > 0, 15000, 500);
    ...
    const setInputs = async (mode) => evaluate(`(() => {
    ...
    const runAndWaitCard = async () => {
      ...
      return poll(async () => evaluate(`document.body.textContent.includes("Total 20-year cost")`), RUN_TIMEOUT_MS);
    };
    ...
    gate("hero CTA present", await evaluate(`document.body.textContent.includes("Start a Free Estimate")`));
    if (!(await chooseHonolulu())) throw new Error("city select failed; aborting run");
    const tariff = await setInputs("gridtie");
    gate("positive grid price", Number.isFinite(tariff) && tariff > 0, `$${tariff}/kWh`);
    gate("result card has Total 20-year cost", await runAndWaitCard());
    gate("savings chart heading present",
      await evaluate(`document.body.textContent.includes("What does solar really save you?")`));
    gate("no savings-unavailable fallback",
      !(await evaluate(`document.body.textContent.includes("Savings data unavailable")`)));
    const canvasBox = await evaluate(`(() => { const c = document.getElementById("cumCostCanvas"); if (!c) return null; const r = c.getBoundingClientRect(); return { w: r.width, h: r.height }; })()`);
    gate("cumCostCanvas non-zero", !!canvasBox && canvasBox.w > 0 && canvasBox.h > 0, JSON.stringify(canvasBox));
    const caption = await evaluate(`(document.getElementById("cumCostCaption")?.textContent || "").trim().length`);
    gate("cumCostCaption explains running cost", (caption || 0) > 20, `${caption} chars`);

    // ── Custom-cut slider re-slice (the incremental engine path) ──────
    console.log("SMOKE      ── bill-cut slider re-slice ──");
    const sliderShown = await evaluate(`(() => { const r = document.getElementById("cutSliderRow"); return r && getComputedStyle(r).display !== "none"; })()`);
    if (sliderShown) {
      await evaluate(`(() => { const s = document.getElementById("cutSlider"); s.value = "82"; s.dispatchEvent(new Event("input", { bubbles: true })); s.dispatchEvent(new Event("change", { bubbles: true })); return true; })()`);
      const relabeled = await poll(async () =>
        (await evaluate(`document.getElementById("cutSliderVal")?.textContent || ""`)).includes("82"), 30000, 1000);
      gate("slider re-labels to 82%", relabeled);
      const stillCard = await poll(async () =>
        (await evaluate(`document.body.textContent`)).includes("Total 20-year cost"), 120000);
      gate("card survives slider re-slice", stillCard);
    } else {
      gate("cut slider visible after run", false, "row hidden — cannot exercise incremental path");
    }
    ...
    const leaflet = await poll(async () => evaluate(`typeof window.L !== "undefined"`), 30000, 1000);
    gate("Leaflet loads (script-src)", leaflet);
    const mapInit = await poll(async () =>
      evaluate(`!!document.querySelector(".leaflet-container")`), 30000, 1000);
    ...
    gate("no other console/page errors", seen.length - csp.length === 0,
      seen.filter((e) => !isCsp(e)).slice(0, 3).join(" | "));
    if (seen.length > 3) console.log(`SMOKE      ...${seen.length} total error lines (first 3 shown above)`);
  } catch (e) {
    gate("smoke run completed", false, String((e && e.message) || e).slice(0, 300));
  } finally {
