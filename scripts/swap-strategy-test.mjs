...
// in CI. `npm test` scopes to tests/ and will NOT pick this up. Re-pinned
// Sept 2026 to the unified engine; re-run to re-pin when pricing or sizing
// economics change. Pricing figures are snapshots by design, not a CI floor.
...
function metrics(s, chem, pvKw, battKwh, capScale) {
  ...
  return {
    cycYr: sim.cyclesEquivalent,
    meanSwing: swingSum / ext.min.length,
    minUsable,
  };
}
...
console.log(
  `Swap-strategy test — real engine, offline fixtures, ${HORIZON_YEARS}-yr horizon, ${DAILY_KWH} kWh/day, cut-${CUT * 100}% (grid-tie)\n`,
);
...
console.log("— Honolulu LFP: oversize is swap-free at ~neutral cost —");
{
  ...
  check(
    `engine pick is the tight 1-swap bank (batt ${pick.battKwh} kWh, ${pickCost.replacements} swap, life ${(CHEMISTRIES.lfp.cyclesTo80 / pickM.cycYr).toFixed(1)} yr)`,
    Math.abs(pick.battKwh - 5) <= 0.5 &&
      pickCost.replacements === 1 &&
      pickM.cycYr > 550,
  );
  const x2 = rows.find((r) => r.m === 2);
  check(
    `2x bank halves cycles/yr (~${pickM.cycYr.toFixed(0)} -> ~${x2.metrics.cycYr.toFixed(0)})`,
    x2.metrics.cycYr / pickM.cycYr >= 0.45 &&
      x2.metrics.cycYr / pickM.cycYr <= 0.55,
  );
  check(
    `2x bank halves mean daily swing (~${(pickM.meanSwing * 100).toFixed(0)}% -> ~${(x2.metrics.meanSwing * 100).toFixed(0)}%)`,
    x2.metrics.meanSwing / pickM.meanSwing >= 0.4 &&
      x2.metrics.meanSwing / pickM.meanSwing <= 0.6,
  );
  check(
    `2x bank reaches 0 swaps at ~same 20-yr cost ($${pickCost.total} -> $${x2.cost.total})`,
    x2.cost.replacements === 0 &&
      Math.abs(x2.cost.total - pickCost.total) < 100,
  );
}
...
);
{
  ...
  check(
    `engine pick carries ${pickCost.replacements} swaps ($${pickCost.total}) with a matching note (${pick.oversizeScenario})`,
    pickCost.replacements === 0 &&
      pick.oversizeScenario === "zero_swap_natural",
  );
  for (const m of [1.5, 2, 3]) {
    const row = rows.find((r) => r.m === m);
    check(
      `${m}x bank (${row.battKwh} kWh, ${row.cost.replacements} swaps) only adds cost ($${row.cost.total} vs pick $${pickCost.total})`,
...
console.log("\n— Honolulu AGM: oversize never pays —");
{
  ...
  check(
    `pick is swap-capped (${pickCost.replacements} swaps, life ${(CHEMISTRIES.agm.cyclesTo80 / h.agm.pickM.cycYr).toFixed(1)} yr)`,
    pickCost.replacements >= 8,
  );
  const x2 = rows.find((r) => r.m === 2),
    x3 = rows.find((r) => r.m === 3);
  check(
    `2x still caps at ${x2.cost.replacements} swaps`,
    x2.cost.replacements >= 7,
  );
  check(
    `3x costs ~$${x3.cost.total - pickCost.total} more — extra capex never recovers`,
    x3.cost.total > pickCost.total + 15000,
  );
}
...
console.log("\n— London LFP: cloudy → the pick is already 0-swap —");
{
  ...
  check(
    `pick already has ${pickCost.replacements} swaps (gentle ~${l.lfp.pickM.cycYr.toFixed(0)} cycles/yr)`,
    pickCost.replacements === 0,
  );
  const x15 = rows.find((r) => r.m === 1.5);
  check(
    `oversizing only adds cost (+$${x15.cost.total - pickCost.total})`,
    x15.cost.total > pickCost.total + 500,
  );
}
console.log(
...
);
{
  ...
  check(
    `pick carries ${pickCost.replacements} swaps ($${pickCost.total})`,
    pickCost.replacements === 0,
  );
  const x2 = rows.find((r) => r.m === 2);
  check(
    `2x bank (${x2.battKwh} kWh, ${x2.cost.replacements} swaps) only adds cost ($${x2.cost.total} vs $${pickCost.total})`,
