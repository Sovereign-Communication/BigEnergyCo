...
if (!csvPath || !Number.isFinite(lat)) {
  console.error(
    "usage: node scripts/validate-against-sheet.mjs <sheet.csv> <lat> <lon> [years]",
  );
  process.exit(1);
}
...
if (sheetVals.length % 8760 !== 0 && sheetVals.length % 8784 !== 0) {
  console.error(
    "Warning: row count does not look like whole years of hourly data.",
  );
}
...
if (e1kw.length !== sheetVals.length) {
  console.error(
    `Length mismatch: engine ${e1kw.length} vs sheet ${sheetVals.length}. Check years.`,
  );
  process.exit(1);
}
...
for (let yIdx = 0; yIdx < years; yIdx++) {
  const slice = (n, len) =>
    [...n].slice(yIdx * len, (yIdx + 1) * len).reduce((a, b) => a + b, 0) /
    1000;
  const len = Math.floor(e1kw.length / years);
  const engineKwh = slice(e1kw, len);
  const sheetKwh =
    sheetVals.slice(yIdx * len, (yIdx + 1) * len).reduce((a, b) => a + b, 0) /
    1000;
  const dev = Math.abs(engineKwh - sheetKwh) / Math.max(sheetKwh, 1);
  worst = Math.max(worst, dev);
  const mark = dev <= 0.05 ? "PASS" : "FAIL";
  console.log(
    `${mark} year ${startYear + yIdx}: engine ${engineKwh.toFixed(0)} kWh vs sheet ${sheetKwh.toFixed(0)} kWh (${(dev * 100).toFixed(1)}% off)`,
  );
}
console.log(
