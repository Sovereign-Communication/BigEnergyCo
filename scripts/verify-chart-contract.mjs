...
for (const { tier, sizing } of results) {
  ...
  historyTiers.push({ id: tier.id, dailyMin: Array.from(mins, (v) => Math.round(v * 1000) / 10), minPct: Math.max(0, Math.round(minPct)), emptyDays });
}
const history = { startYear: meta.startYear, endYear: meta.endYear, days: Math.ceil(hours.length / 24), tiers: historyTiers };

// === UI SIDE (the exact gate drawSocChart uses) ===
const solvable = history.tiers.filter((t) => t.dailyMin && t.dailyMin.length);
console.log(`tiers sent: ${history.tiers.length}, pass UI filter: ${solvable.length}`);
if (solvable.length !== history.tiers.length || solvable.length === 0) {
  console.log("CONTRACT BROKEN"); process.exit(1);
}
for (const t of solvable) {
  console.log(`OK ${t.id}: ${t.dailyMin.length} daily points, min ${Math.min(...t.dailyMin)}%, emptyDays ${t.emptyDays}`);
}

// === SERVED-BYTES CHECK: does the live worker file still match this contract? ===
for (const base of ["https://sovereign-communication.github.io/BigEnergyCo", "https://bigenergyco.pages.dev"]) {
  const w = Buffer.from(await (await fetch(base + "/assets/js/sizing/sizing-worker.js")).arrayBuffer()).toString("utf8");
  console.log(`${base.includes("pages") ? "pages.dev " : "github.io "} serves current worker: ${w.includes("dailyMinimums(traced.socSeries)")}`);
}
