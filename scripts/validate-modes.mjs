...
console.log(
  `Site ${lat},${lon} · ${dailyKwh} kWh/day · $${tariff}/kWh · ${chemistry} · fetching NASA POWER...`,
);
const t0 = Date.now();
...
console.log(
  `${hours.length} hourly records (${meta.startYear}–${meta.endYear}) in ${((Date.now() - t0) / 1000).toFixed(1)}s`,
);
...
const costPerKwhBatt =
  (landed.battPerKwhUsable[0] + landed.battPerKwhUsable[1]) / 2;
...
// ── Off-grid tiers ──
console.log("\n── OFF-GRID TIERS ──");
for (const { tier, sizing } of sizeAllTiers({
  e1kw,
  loadWh,
  tempsC,
  chemistry,
  ...
  if (!sizing) {
    console.log(`  ${tier.id}: no solution`);
    continue;
  }
  const cost = fullRange(sizing.pvKw, sizing.battKwh);
  const spend = annualGridSpendUsd(dailyKwh, tariff);
  const pbLo = paybackYears(cost.lo, spend),
    pbHi = paybackYears(cost.hi, spend);
  console.log(
    `  ${tier.id}: ${sizing.pvKw} kW + ${sizing.battKwh} kWh · ${money(cost.lo)}–${money(cost.hi)} · payback ${pbLo?.toFixed(2)}–${pbHi?.toFixed(2)} yr`,
  );
  check(
    sizing.result.unmetHours / yrs <= tier.maxUnmetHoursPerYear + 1e-6,
    `${tier.id} exceeds unmet budget`,
  );
  check(
    Number.isFinite(pbLo) && Number.isFinite(pbHi) && pbLo <= pbHi,
    `${tier.id} payback range sane`,
  );
}
...
const t1 = Date.now();
for (const { target, sizing } of sizeAllBillTargets({
  e1kw,
  loadWh,
  tempsC,
  chemistry,
  ...
  if (!sizing) {
    console.log(`  ${target.id}: not reachable`);
    continue;
  }
  const cost = fullRange(sizing.pvKw, sizing.battKwh);
  ...
  const cutPct =
    (1 - sizing.result.importedWh / (dailyKwh * 1000 * hrs24(yrs))) * 100;
  console.log(
  ...
  check(
    cutPct >= target.minFraction * 100 - 0.5,
    `${target.id} misses its cut target (${cutPct.toFixed(1)}%)`,
  );
...
function hrs24(years) {
  return (years * 365 * 24) / 24;
} // days→hours identity, kept for clarity
