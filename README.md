...
| `scripts/validate-modes.mjs`              | Live end-to-end check of both sizing modes vs real NASA data (weekly: `npm run verify:live`, needs network — never part of the offline PR suite)                                             |
...
| `docs/plan/MASTER_PLAN.md`                | **The** plan: immutable (hash-pinned, `npm run plan:check`), amended only via `docs/plan/AMENDMENTS.md`; progress in `docs/plan/LEDGER.jsonl`. Supersedes every other plan doc               |
| `docs/INDEX.md`                           | Doc index: what is current, what is history                                                                                                                                                  |
| `docs/archive/`                           | Twelve superseded plans/audits, moved by P0.2, each with a banner naming why it was archived and where its content went. Not current guidance                                                |
